#!/bin/sh
# build_standalone.sh — builds the portable single-file obsidian-doctor
...
#
#   docker run --rm -v "$PWD/..:/src" -w /src/doctor_gui debian:stable sh packaging/build_standalone.sh
#
...
DOCTOR_GUI_DIR=$(cd "$SCRIPT_DIR/.." && pwd)
...
pyinstaller --onefile --name obsidian-doctor --noconfirm --clean app.py

deactivate

echo
echo "Built: $DOCTOR_GUI_DIR/dist/obsidian-doctor"
